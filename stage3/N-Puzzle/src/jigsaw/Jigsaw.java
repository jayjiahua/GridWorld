...
public class Jigsaw {
    ...
    private void estimateValue(JigsawNode jNode) {
        ...
        for(int index = 1 ; index<dimension*dimension; index++){
            ...
            if(jNode.getNodesState()[index] != 0 && jNode.getNodesState()[index+1] != 0 && jNode.getNodesState()[index] +1 != jNode.getNodesState()[index+1]) {
                s++;
            }
        }
        ...
        for(int index = 1 ; index<= dimension; index++){
            for (int j = 0 ; j < dimension - 1 ; j++) {
                if(jNode.getNodesState()[j * dimension + index] != 0 && jNode.getNodesState()[(j + 1) * dimension + index] != 0 &&
                        jNode.getNodesState()[j * dimension + index] + dimension != jNode.getNodesState()[(j + 1) * dimension + index]) {
                    s++;
                }
            }
        }
        ...
        for (int i = 1 ; i <= dimension * dimension ; i++) {
            ...
            for (int j = 1 ; j <= dimension * dimension ; j++) {
                if (jNode.getNodesState()[i] == endJNode.getNodesState()[j]) {
                    int iRow = (i - 1) / dimension;
                    int iCol = (i - 1) % dimension;
                    int jRow = (j - 1) / dimension;
                    int jCol = (j - 1) % dimension;
                    d += Math.abs(jRow - iRow) + Math.abs(jCol - iCol); 
                    c += Math.pow(iRow - jRow, 2) + Math.pow(iCol - jCol, 2);
                    break;
                }
            }
        }
        ...
        /*      本人呕心沥血测试出来的黄金配比！！！！
                ...
                25000步内绝不失败！！！       
        */
        jNode.setEstimatedValue(7 * s + 6 * d + 3 * c);
